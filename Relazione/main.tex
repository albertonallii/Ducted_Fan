...
fprintf("===================================\n");
	...
	\end{figure}
		...
		\caption{\textit{Dataset} sperimentale ad anello aperto ottenuto tramite stimolazione PRBS. In alto: comandi PWM ai motori; al centro: comandi PWM ai \textit{flap}; in basso: risposta angolare del velivolo misurata dall'IMU. A sopra l'asse di \textit{roll}, a sotto l'asse di \textit{pitch}}
...
Si tratta di valori soddisfacenti per un’identificazione black-box su un sistema dotato di non linearità importanti e testato in condizioni vincolate. La quota residua di varianza non spiegata dal modello è imputabile a effetti che un modello lineare SISO non può catturare integralmente: in particolare l’attrito non lineare sui perni dei flap, l’azione non perfettamente simmetrica delle funi di vincolo e i piccoli accoppiamenti tra gli assi. I modelli identificati devono quindi essere interpretati come approssimazioni locali della dinamica dominante del DPDF nella configurazione sperimentale adottata, sufficienti per la sintesi preliminare dei regolatori PIDF e per la successiva verifica in simulazione.
	...
	\end{figure}
		...
		\caption{Analisi di \textit{cross-validazione} nel dominio del tempo per i due assi. Confronto tra risposta angolare reale del prototipo (linea nera) e uscita stimata dal modello LTI (linea colorata).Sopra l'asse di \textit{roll} (Fit $77.85\%$), sotto l'asse di \textit{pitch} (Fit $77.67\%$)}
	...
	\paragraph{Azione proporzionale.} Il termine $K_p \, e(t)$ produce una correzione istantanea proporzionale all'entità dell'errore: quanto più il drone è inclinato, tanto più ampia è la deflessione richiesta ai \textit{flap}. È l'azione di richiamo principale, ma da sola non è sufficiente. In presenza di un disturbo costante --- come il momento ribaltante gravitazionale del baricentro eccentrico --- l'azione proporzionale lascia infatti un \textbf{errore residuo a regime} (\textit{steady-state error}): per generare un momento correttivo permanente che bilanci il disturbo costante, è necessario un errore non nullo che alimenti il termine proporzionale.
	
	\paragraph{Azione integrale.} Il termine $K_i \int e\, d\tau$ accumula nel tempo l'errore e ne integra la storia passata. La sua funzione è proprio quella di \textbf{annullare l'errore a regime}: finché permane un errore, anche piccolo, l'integrale continua a crescere aumentando la correzione, fino a quando il momento correttivo eguaglia esattamente il disturbo costante e l'errore si azzera. Nel caso del \textit{DPDF} questa azione è essenziale per compensare lo sbilanciamento statico del baricentro. Il prezzo da pagare è il rischio di \textit{windup}: se l'attuatore satura (i \textit{flap} raggiungono il limite di $\pm 30\degree$) e l'errore persiste, l'integrale può accumulare valori molto elevati, generando sovraelongazioni e ritardi di recupero. Per questo motivo l'azione integrale è dotata di un meccanismo \textbf{anti-windup} a saturazione, descritto nel seguito.
	...
	Il regolatore non opera nel continuo ma viene eseguito nel firmware come algoritmo a tempo discreto, all'interno dell'\textit{interrupt} deterministico del \textit{Timer 7} alla frequenza di 50~[Hz] ($T_s = 0.02$~[s]). L'implementazione adottata è in \textbf{forma posizionale} (\textit{positional form}): a ogni passo di campionamento $k$ l'azione di controllo viene \textit{ricalcolata per intero} a partire dall'errore corrente, dall'accumulatore integrale e dalla derivata filtrata. Questa scelta si distingue dalla forma \textit{incrementale} (\textit{velocity form}), nella quale si calcola invece l'incremento $\Delta u[k]$ rispetto al comando precedente; la forma posizionale è stata preferita per la maggiore trasparenza nella gestione esplicita della saturazione dell'attuatore e del termine anti-windup.
	...
	Il comando $u[k]$, espresso in gradi di deflessione del \textit{flap}, viene poi convertito nel corrispondente valore CCR tramite la costante di conversione $K_\text{CCR} = 10.0$~[CCR/deg] (Sezione~\ref{sec:pre_elaborazione}) prima di essere inviato al timer di generazione del segnale PWM. Poiché i guadagni sono stati identificati lavorando in unità fisiche di gradi (Sezione~\ref{sec:architettura_modello}), i valori $K_p$, $K_i$, $K_d$ sintetizzati in MATLAB sono direttamente trasferibili nei campi del file \texttt{config.c} senza ulteriori riscalature.
...
sys_ctrl = feedback(C_pid, sys_asse);        % sforzo di controllo
	...
	\begin{figure}[H]
		...
		\caption{Analisi della risposta al gradino ad anello chiuso (sopra) e del corrispondente sforzo di controllo dei servomotori (sotto). Il picco iniziale dell'azione di controllo si attesta entro il vincolo fisico di $\pm 30\degree$}
