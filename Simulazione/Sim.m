...
KV      = 1400;
ARR     = 14999;
Ixx = 0.0062; Iyy = 0.0062; Izz = 0.0100;
d_cg_x  = 0.0028;
d_cg_y  = 0.0038;
...
%% Stato dinamico
S = init_state(ALT_EQ, H_CYL, TRAIL_MAX, Z_CYL_CENTER_EQ);
...
fprintf('\nSimulatore DPDF avviato.\n');
fprintf('Premere START e regolare gli slider CCR per pilotare il drone.\n\n');


%% Funzioni

function S = init_state(ALT_EQ, H_CYL, TRAIL_MAX, Z_CYL_CENTER_EQ)
    S.t       = 0;
    S.x       = 0;
    S.y       = 0;
    S.z       = Z_CYL_CENTER_EQ;
...
function btn_reset(fig)
    ...
    btn_stop(fig);
    C = fig.UserData.C;
    fig.UserData.S = init_state(C.ALT_EQ, C.H_CYL, C.TRAIL_MAX, C.Z_CYL_CENTER_EQ);
    ud = fig.UserData;
