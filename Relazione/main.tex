...
\begin{document}
	...
	\begin{titlepage}
		\begin{center}
			...
			\linespread{1}
			\includegraphics[width=5cm]{images/logoUnivpm.jpg}\\[0.8cm]
			\linespread{1}
	...
	L'ESC \textit{Turnigy Plush 40A} offre la possibilità di configurare molteplici parametri operativi per l'ottimizzazione del controllo motore. Nell'ambito del presente progetto, la programmazione di entrambi gli ESC è stata eseguita avvalendosi della \textit{Turnigy Programming Card}, al fine di tararne il comportamento in funzione delle specifiche esigenze aerodinamiche e meccaniche del \textit{DPDF}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{figures/02_hardware/esc_programming_setup.jpg}
		\caption{Schema dei collegamenti per la programmazione dell'ESC}
		\label{fig:esc_programming_setup}
	\end{figure}
	
	Per procedere alla configurazione, è necessario predisporre i collegamenti elettrici illustrati in Figura~\ref{fig:esc_programming_setup}. La \textit{Programming Card} (visibile in Figura~\ref{fig:programming_card}) richiede una tensione di alimentazione compresa nell'intervallo $4.8 \div 6.0$ [V]. La navigazione all'interno del menù dei parametri si effettua mediante i pulsanti direzionali (\textit{up/down} e \textit{left/right}), mentre l'avvenuta memorizzazione del valore selezionato viene confermata dall'accensione del LED \textit{connecting}.\\
	Di seguito vengono analizzati i principali parametri di configurazione e le relative impostazioni adottate per i test:
	
	\paragraph{Brake (Freno motore)}\mbox{}\\
	Questo parametro definisce il comportamento del rotore in assenza di segnale PWM di trazione. Poiché le sessioni di collaudo sono state condotte all'interno di una gabbia di sicurezza, tale funzione non ha assunto una rilevanza critica per la dinamica del sistema; è stata pertanto disabilitata impostandola su \textbf{OFF}.
	
	\paragraph{Battery Type}\mbox{}\\
	Tale voce istruisce l'ESC riguardo alla chimica dell'accumulatore impiegato (\textit{LiPo} o \textit{NiMH}), requisito fondamentale per una corretta gestione delle soglie di protezione. Impiegando il sistema pacchi batteria \textit{Tattu LiPo}, è stata logicamente selezionata l'opzione \textbf{Li-xx}.
	
	\paragraph{Low Voltage Protection Mode (Tipologia di Cut-off)}\mbox{}\\
	Il parametro stabilisce la reazione dell'ESC qualora la tensione di alimentazione scenda al di sotto del valore limite prefissato. Al fine di scongiurare arresti improvvisi dei motori durante le fasi di test, con conseguente perdita repentina di portanza, si è optato per la modalità \textbf{Soft Cut-Off}, la quale garantisce una riduzione progressiva e controllata della potenza erogata.
	
	\paragraph{Cut Off Voltage (Tensione di Cut-off)}\mbox{}\\
	...
	\begin{itemize}
		...
		\item High: 3.3 [V].
	\end{itemize}
	È stata scelta l'impostazione \textbf{Medium}, che nel caso di una batteria LiPo a 4 celle corrisponde a una soglia complessiva di \textbf{12.6 [V]}.
	
	\paragraph{Start Mode}\mbox{}\\
	Regola la rampa di accelerazione iniziale del motore nel transitorio da fermo a regime. Durante le prove preliminari, è stata impiegata la configurazione \textbf{Very Soft Start} per minimizzare le sollecitazioni meccaniche sulla struttura, evitare reazioni brusche e ridurre i disturbi di misura durante i test in gabbia. Nelle fasi successive, specificatamente dedicate alla taratura dell'algoritmo PID per il controllo dell'imbardata (\textit{yaw}), è stata invece ripristinata la modalità \textbf{Normal} per garantire una risposta dinamica più reattiva.
	
	\paragraph{Timing Mode}\mbox{}\\
	Rappresenta l'anticipo temporale con cui l'ESC commuta le fasi statoriche rispetto alla posizione angolare del rotore. Un anticipo ridotto privilegia l'efficienza energetica e la regolarità di rotazione, mentre un anticipo elevato massimizza la potenza meccanica erogata e i regimi di rotazione. Per il \textit{DPDF} è stata preferita l'impostazione \textbf{Medium Timing}, in quanto ritenuta il miglior compromesso tecnico tra prestazioni dinamiche ed efficienza complessiva.
	
	\paragraph{Music / Li-Po Cells}\mbox{}\\
	Questa funzione riveste una valenza puramente estetica (notifiche acustiche all'accensione) e non incide sulle prestazioni cinematiche o elettriche del sistema; pertanto, non è stata abilitata.
	
	\paragraph{Governor Mode}\mbox{}\\
	Questa logica di controllo è preposta al mantenimento di un regime di rotazione costante al variare del carico meccanico applicato. Non risultando necessaria per le logiche di controllo implementate nel presente lavoro, l'opzione è stata mantenuta su \textbf{OFF}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.45\textwidth]{figures/02_hardware/turnigy_programming_card.jpg}
		\caption{Turnigy Programming Card}
		\label{fig:programming_card}
	\end{figure}
	\begin{center}
		\footnotesize\textit{L'immagine riportata sopra non è riferita alla configurazione attuale degli ESC, ma ha esclusivamente scopo illustrativo.}
	\end{center}
	...
	L'architettura di controllo del \textit{DPDF} è organizzata in \textbf{tre catene logiche distinte}, ciascuna dedicata a una specifica dinamica del velivolo. A livello implementativo, queste tre catene istanziano complessivamente \textbf{quattro regolatori PID} indipendenti, perché il ramo di assetto richiede due regolatori separati per \textit{roll} e \textit{pitch}. La scomposizione adottata è la seguente:
	\begin{itemize}
	...
	\subsection{PID discreto adottato}
	Il regolatore PID implementato in \texttt{pid.c} adotta la forma incrementale a tempo discreto in cui i tre contributi sono calcolati separatamente e poi sommati. Dato un errore $e[k] = r[k] - y[k]$ tra riferimento $r$ e misura $y$ al passo $k$-esimo, il comando viene calcolato come:
	\begin{align}
		P[k] &= K_p \cdot e[k] \\
		I[k] &= I[k-1] + K_i \cdot T_s \cdot e[k] \\
		D[k] &= \frac{K_d}{T_s}\bigl(e[k] - e[k-1]\bigr) \\
		u_{raw}[k] &= u_0 + P[k] + I[k] + D[k]
	\end{align}
	dove $T_s$ è il periodo di campionamento del ramo, $u_0$ è un eventuale \textit{offset} di funzionamento (utile per il ramo quota, in cui un PWM diverso da zero è necessario anche a errore nullo per sostenere il peso) e $u_{raw}[k]$ è il comando grezzo prima delle saturazioni.\\
	Il periodo di campionamento $T_s$ è coerente con la frequenza di aggiornamento del ramo a cui il PID appartiene: \textbf{10 [ms]} per i regolatori di \textit{roll}, \textit{pitch} e \textit{yaw}, allineati al ramo IMU/servo; \textbf{coerente con la disponibilità di una nuova misura del ToF} per il PID di quota.
	
	\subsection{Saturazioni hardware e \textit{anti-windup}}
	Il comando grezzo $u_{raw}[k]$ generato dal PID non può essere applicato direttamente all'attuatore: motori e servomotori operano entro intervalli finiti e fissi, definiti come costanti in \texttt{config.h} (\texttt{UPPER\_LIMIT\_MOTOR}, \texttt{LOWER\_LIMIT\_MOTOR}, \texttt{UPPER\_LIMIT\_SERVO}, \texttt{LOWER\_LIMIT\_SERVO}). Il firmware effettua dunque una saturazione esplicita prima di scrivere il valore nel \textit{Capture-Compare Register} del relativo \textit{timer}:
	\begin{equation}
		u[k] = \mathrm{clamp}\bigl(u_{raw}[k], u_{min}, u_{max}\bigr).
	\end{equation}
	Per evitare il fenomeno dell'\textit{integral windup} --- ovvero l'accumulo indefinito del termine integrale quando l'attuatore è saturato e non riesce a ridurre l'errore --- è stata implementata la tecnica del \textit{clamping}: quando il comando grezzo eccede i limiti e l'errore tende a far crescere ulteriormente il termine integrale, l'aggiornamento dell'integrale viene sospeso fino a quando l'attuatore non rientra nella zona lineare. Questa scelta, semplice ma efficace, evita le sovraelongazioni che si verificherebbero al rientro dalla saturazione, fenomeno particolarmente indesiderabile in un sistema in cui la dinamica meccanica è già intrinsecamente lenta.
	...
	\begin{itemize}
		...
		\item \textbf{Canale di comunicazione}: la trasmissione avviene su periferica \textit{USART3}, esposta all'esterno attraverso il \textit{Virtual COM Port (VCP)} dell'\textit{ST-LINK/V3E} integrato sulla scheda. Per garantire che le stringhe di telemetria (denso \textit{payload} di 15 campi) vengano trasmesse interamente entro il periodo di campionamento, il \textit{baudrate} è stato impostato a \textbf{230\,400 [bps]}. Questo valore è significativamente più elevato dei 115\,200 \textit{baud} comunemente adottati per il \textit{debug} testuale, e si è dimostrato necessario per sostenere il traffico a 50 [Hz] senza saturare il canale.
...
duration     = 30;       
	...
	\begin{itemize}
		...
		\item \texttt{DT\_TARGET = 0.02} sancisce il passo di campionamento temporale dell'intero sistema. Coordinare la telemetria alla stessa frequenza a cui opera il PID nel firmware (50 [Hz]) è una scelta metodologica essenziale per evitare effetti di \textit{aliasing} e sfasamenti nei modelli identificati.
	\end{itemize}
...
if (t - nextCmdTime) > DT_TARGET
	...
	Se il tempo corrente $t$ supera abbondantemente \texttt{nextCmdTime}, segno di un blocco temporaneo del sistema operativo, lo \textit{script} non cerca di compensare il ritardo accumulando comandi arretrati. Riallinea invece la variabile \texttt{nextCmdTime} sul tempo corrente, garantendo che i comandi successivi mantengano lo spaziamento nominale $\Delta t = 20$ [ms]. La conseguenza è la perdita di qualche campione in corrispondenza del \textit{glitch}, ma la coerenza temporale del resto della prova viene preservata.
...
if count(line, ",") == 14
	...
	Questo approccio garantisce l'integrità dei dati salvati. Se un disturbo elettromagnetico sui cavi del laboratorio corrompe un pacchetto UART troncandolo a metà, il conteggio delle virgole fallisce: lo \textit{script} scarta la riga, classificandola come \texttt{badLines}, e mantiene il file \texttt{.csv} libero da valori anomali (\texttt{NaN}) che farebbero fallire le successive procedure di identificazione in \texttt{tfest}.
...
function keyKill(~, event)
	...
	\begin{itemize}
		...
		\item \textbf{prove 5.x} --- eccitazioni combinate e sequenze pseudo-casuali (PRBS) su uno o entrambi gli assi.
	\end{itemize}
	...
	\subsection{Sequenze PRBS}
	Le prove contrassegnate dai codici 5.2, 5.3 e 5.4 implementano sequenze binarie pseudo-casuali (PRBS, \textit{Pseudo-Random Binary Sequence}). In queste modalità il comando inviato ai \textit{flap} non segue una legge matematica continua, ma salta in modo istantaneo tra livelli discreti definiti a partire dalle costanti \texttt{STEP\_SMALL} e \texttt{STEP\_MEDIUM}, a intervalli temporali regolari (ogni 0.5 [s]).
	
	Il PRBS è lo standard nell'identificazione \textit{black-box} dei sistemi dinamici. Da un punto di vista spettrale, un salto pseudo-casuale tra livelli discreti approssima il comportamento di un rumore bianco entro la banda di interesse: il segnale contiene energia su un'ampia gamma di frequenze, permettendo di stimolare contemporaneamente tutti i modi della dinamica e di alimentare in modo efficiente gli algoritmi di identificazione parametrica come \texttt{tfest} (descritto nel Capitolo 5).
	...
	\begin{lstlisting}[language=Matlab, caption={Generazione della sequenza PRBS roll (case 21)}]
case 21
...
fprintf("===================================\n");
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figures/05_modello/dataset_prbs.png}
...
[best_fit, best_idx] = max([fitA, fitB, fitC]);
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figures/05_modello/cross_validation.png}
...
sys_ctrl = feedback(C_pid, sys_asse);        % sforzo di controllo
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figures/05_modello/step_response.png}
...
function [Fx, Fy, Fz, Mx, My] = rope_forces(S, C)
	...
	Per il primo collaudo sul prototipo fisico si è scelto di adottare una configurazione di sicurezza, ottenuta riducendo i guadagni del modello LTI di un fattore approssimativamente pari a due:
	\begin{equation}
		K_{p,\text{safe}} = 10.0, \quad
		K_{i,\text{safe}} = 15.0, \quad
		K_{d,\text{safe}} = 1.0.
		\label{eq:derating}
	\end{equation}
	Questi valori sono stati prima validati nel simulatore (producendo la risposta mostrata in Figura~\ref{fig:simulazione_hover}) e successivamente impiegati come punto di partenza nel firmware. La pratica del \textit{de-rating} è una scelta metodologica conservativa coerente con la prudenza richiesta nella validazione di sistemi non lineari: i guadagni più aggressivi suggeriti da \texttt{pidtune} possono essere reintrodotti incrementalmente in fase di taratura fine sul prototipo, una volta acquisita confidenza con il comportamento reale del sistema.
