...
Per soddisfare questi rigorosi requisiti, il firmware è stato strutturato secondo un'architettura modulare ed \textit{event-driven}. È infatti imprescindibile che il microcontrollore garantisca:
\begin{enumerate}
    \item Un campionamento deterministico delle grandezze di stato (angoli di Eulero forniti dalla IMU BNO-055 e quota fornita dal ToF VL53L1X), con \textit{jitter} di fase contenuto;
    \item Un sistema di telemetria ad alta frequenza, capace di trasmettere a 50 [Hz] su porta seriale a 230\,400 [baud] lo stato completo del sistema, inclusi angoli, quota, comandi PWM applicati agli attuatori, contatori di errore UART e flag di stato;
    \item Un protocollo di comando sicuro, affinché si possa testare il drone fisico in sicurezza, per evitare possibili malfunzionamenti e comportamenti anomali;
    \item Una gestione robusta della comunicazione UART, con meccanismi di ricezione, parsing, diagnostica degli errori e ripristino della comunicazione;
    \item Un'infrastruttura di controllo flessibile, pronta a recepire i guadagni ottimali del PID calcolati in simulazione, per procedere alla validazione sul prototipo fisico in condizioni di massima sicurezza.
\end{enumerate}
...
%--------------------------------------------------------------
\section{Architettura software event-driven}
L'infrastruttura firmware sviluppata per il DPDF si fonda su un'architettura \textit{event-driven}, ovvero guidata dagli eventi temporali e hardware. A differenza dei paradigmi puramente sequenziali, basati sul \textit{polling} continuo delle periferiche, l'architettura adottata fa sì che l'unità di calcolo intervenga quando si verifica un evento rilevante: la disponibilità di una nuova misura dal sensore di quota, la scadenza di un timer periodico, l'arrivo di un comando dalla porta seriale oppure la pressione di un pulsante fisico di emergenza.

Per garantire questa reattività, le \textit{Interrupt Service Routine} (ISR) e le callback HAL associate a timer e periferiche sono state mantenute estremamente leggere. Il loro compito principale è aggiornare flag logiche, contatori o variabili di stato, senza eseguire calcoli onerosi o accessi prolungati alle periferiche. L'elaborazione vera e propria — acquisizione dati via $I^{2}C$, compensazione e filtraggio della quota, aggiornamento dei segnali PWM, gestione dei comandi e composizione della telemetria — viene invece eseguita nel ciclo principale \texttt{while(1)}, che monitora lo stato delle flag ed esegue i blocchi computazionali solo quando il rispettivo sottosistema lo richiede.
...
%--------------------------------------------------------------
\section{Gestione del microcontrollore dual-core}
Il microcontrollore \textit{STM32H745ZI-TQ6}, descritto nel Capitolo~\ref{ch:hardware}, è caratterizzato da un'architettura \textit{dual-core} che integra un processore \textit{Arm Cortex-M7} e un coprocessore \textit{Arm Cortex-M4}. Sebbene l'intera logica di controllo sia eseguita dal Cortex-M4, anche il Cortex-M7 riveste un ruolo essenziale nella fase di avvio del sistema.

Per ragioni architetturali interne al microcontrollore, la configurazione del clock e l'inizializzazione della \textit{Power Domain} principale sono di esclusiva competenza del Cortex-M7, in quanto core primario. Il Cortex-M4 resta dunque in attesa, in modalità a basso consumo (\textit{Wait For Event}, WFE), fino a quando il Cortex-M7 non gli segnala il completamento di tale configurazione tramite un \textit{Hardware Semaphore} (HSEM). Solo dopo aver ricevuto questo segnale, il Cortex-M4 esce dallo stato di attesa e procede con l'inizializzazione delle proprie periferiche e con l'avvio del main loop di controllo.
...
Per favorire la leggibilità, la manutenibilità e la verificabilità del firmware, il codice sorgente è stato organizzato in moduli funzionalmente coesi. Ogni modulo è composto, dove previsto, da un \textit{header file} \texttt{.h}, che dichiara l'interfaccia pubblica del modulo, e da un \textit{source file} \texttt{.c}, che ne contiene l'implementazione. Le configurazioni di basso livello generate da STM32CubeMX sono state mantenute separate dai moduli applicativi, in modo da non appesantire il file \texttt{main.c} con dettagli implementativi non direttamente legati alla logica del sistema.

I principali moduli applicativi sviluppati sono i seguenti:
\begin{itemize}
    \item \textbf{main.c} — contiene la sequenza di inizializzazione del sistema, la procedura di \textit{safe startup}, il ciclo operativo principale basato sulle flag di evento e la chiamata coordinata agli altri moduli. Al suo interno sono inoltre gestite le callback principali associate a timer, GPIO e UART. In particolare, il file \texttt{main.c} coordina la ricezione e il parsing dei comandi provenienti da MATLAB, l'aggiornamento dei PWM, la composizione della telemetria e la gestione delle principali condizioni di sicurezza.

    \item \textbf{config.h e config.c} — raccolgono in un unico punto i principali parametri numerici del firmware, tra cui i limiti PWM per motori e servomotori, la posizione neutra dei servo, i fattori di conversione angolo–PWM e la struttura di configurazione dei regolatori. Centralizzare questi valori rende esplicito il confine tra comando richiesto, comando ammissibile e limite imposto dall'hardware reale, evitando la dispersione di valori numerici direttamente all'interno del codice.

    \item \textbf{pwm.h e pwm.c} — incapsulano la gestione dei segnali PWM destinati ai motori e ai servomotori. Il modulo espone funzioni di alto livello per l'avvio dei canali PWM, l'aggiornamento dei comandi e lo spegnimento degli attuatori. In questo modo la logica applicativa può richiamare funzioni come \texttt{set\_pwm\_motors()} e \texttt{set\_pwm\_servos()} senza intervenire direttamente sui registri dei timer.

    \item \textbf{pid.h e pid.c} — implementano la struttura del regolatore PID discreto e alcune funzioni di supporto, tra cui il filtro mediano utilizzato nella catena di acquisizione della quota. Nel firmware attuale le strutture PID vengono inizializzate e mantenute nel codice come predisposizione per successive integrazioni, ma non vengono utilizzate per generare i PWM durante le prove open-loop comandate da MATLAB.

    \item \textbf{imu.h e imu.c} — costituiscono un livello di astrazione sopra il driver del sensore BNO055. Il modulo si occupa dell'inizializzazione dell'IMU, della lettura degli angoli di Eulero e della gestione del riferimento iniziale, così da ottenere valori relativi di roll, pitch e yaw rispetto alla posa iniziale del DPDF.

    \item \textbf{tof.h e tof.c} — contengono le funzioni di supporto associate alla misura di quota fornita dal VL53L1X. In particolare, il modulo implementa la compensazione geometrica della distanza misurata dal sensore rispetto all'inclinazione del velivolo, utilizzando gli angoli di roll e pitch forniti dall'IMU.

    \item \textbf{Driver BNO055 e VL53L1X} — oltre ai moduli applicativi, il progetto include i driver necessari all'interfacciamento con i sensori. I file relativi al BNO055 e alla libreria VL53L1X forniscono le funzioni di basso livello per la comunicazione con i dispositivi, mentre i moduli \texttt{imu.c} e \texttt{tof.c} ne semplificano l'utilizzo all'interno della logica del firmware.
\end{itemize}
...
La configurazione hardware del Cortex-M4 è stata generata tramite STM32CubeMX a partire dal file di progetto \texttt{.ioc}, ed è stata mantenuta coerente con i collegamenti pin-to-pin descritti nel Capitolo~\ref{ch:hardware}. Si descrivono di seguito le periferiche principali impiegate dal firmware.

\subsection{Timer}
I timer impiegati svolgono ruoli funzionalmente distinti:
\begin{itemize}
    \item \textbf{TIM2}: configurato in modalità \textit{PWM Generation}, genera sui canali CH1 e CH2 i segnali di controllo dei servomotori di roll e pitch. La frequenza di funzionamento è pari a 50 [Hz], coerentemente con le specifiche dei servomotori HS-82MG.

    \item \textbf{TIM4}: anch'esso configurato in modalità \textit{PWM Generation}, genera sui canali CH2 e CH3 i segnali destinati agli ESC Turnigy Plush 40A che pilotano i due motori brushless. Anche in questo caso la frequenza è pari a 50 [Hz], come richiesto dagli ESC stessi.

    \item \textbf{TIM3}: utilizzato come base temporale per la generazione periodica dei campioni di telemetria. A ogni interrupt viene aggiornato il tempo telemetrico e viene incrementato il numero di campioni pendenti da trasmettere. Questa scelta permette di separare la temporizzazione della telemetria dalla trasmissione effettiva via UART, che viene invece gestita nel ciclo principale.

    \item \textbf{TIM6}: impiegato per la sequenza temporizzata di \textit{safe startup} e per il pilotaggio diagnostico dei LED integrati sulla scheda.

    \item \textbf{TIM7}: utilizzato come \textit{timebase} per la generazione periodica dell'interrupt che abilita l'aggiornamento del ramo IMU–attuatori. La sua scadenza imposta la flag \texttt{actuate\_servo\_control}, che il main loop intercetta per eseguire la lettura del BNO055, aggiornare i valori operativi dei servomotori e applicare i PWM agli attuatori. Nella configurazione di test open-loop i PWM sono ricevuti da MATLAB; la stessa struttura temporale rimane tuttavia compatibile con la successiva integrazione dei PID di assetto, tarati a partire dal modello identificato.
\end{itemize}

\begin{lstlisting}[caption={Callback periodica dei timer utilizzati per safe startup, ramo IMU–attuatori e telemetria.}, label={lst:nome}]
...
\begin{equation}
    f_{PWM} = \frac{f_{clk}}{(Prescaler + 1) \cdot (ARR + 1)}
    \label{eq:timer_freq}
\end{equation}
Nel caso dei timer PWM, tale relazione determina la frequenza del segnale generato; nel caso dei timer usati come \textit{timebase}, determina invece la frequenza con cui vengono generati gli interrupt periodici.

\subsection{Periferiche di comunicazione}
Il firmware utilizza due tipologie principali di periferiche di comunicazione:
\begin{itemize}
    \item \textbf{USART3}: configurata in modalità \textit{full-duplex} asincrona a 230\,400 [baud], con 8 bit di dato, 1 bit di stop e nessuna parità. Questo canale costituisce il collegamento bidirezionale con il PC ospite: in trasmissione veicola la telemetria seriale, mentre in ricezione raccoglie i comandi inviati da MATLAB. La scelta di un baud rate elevato è motivata dalla necessità di sostenere sia il flusso telemetrico sia l'invio asincrono dei comandi, considerando che ogni pacchetto trasmesso è costituito da una stringa CSV di lunghezza variabile.

    \item \textbf{$I^{2}$C1 e $I^{2}$C2}: configurati in \textit{Fast Mode} a 400 [kHz], sono dedicati rispettivamente alla comunicazione con il sensore BNO055 e con il sensore VL53L1X. L'impiego di due bus distinti, anziché di un unico bus condiviso, evita possibili conflitti e permette di gestire i due sensori in modo indipendente.
\end{itemize}

La gestione degli interrupt associati a queste periferiche è affidata al \textit{Nested Vectored Interrupt Controller} (NVIC), configurato con livelli di priorità coerenti con la criticità temporale di ciascuna sorgente di evento.

%--------------------------------------------------------------
\section{Gestione dei sensori}

\subsection{VL53L1X — Acquisizione di quota}
L'acquisizione della quota avviene tramite il sensore Time-of-Flight VL53L1X, descritto nel Capitolo~\ref{ch:hardware}. Il firmware utilizza il driver ufficiale fornito da STMicroelectronics, integrato attraverso un sottile livello di adattamento alle funzioni HAL.

La sequenza di inizializzazione del sensore prevede: il rilascio del segnale XSHUT, il caricamento del firmware interno tramite \texttt{VL53L1\_WaitDeviceBooted()}, l'inizializzazione tramite \texttt{VL53L1\_DataInit()}, l'impostazione della modalità \textit{Short ranging} e del \textit{Timing Budget} a 33 [ms], e infine l'avvio della misura continua tramite \texttt{VL53L1\_StartMeasurement()}.

Per non bloccare il microcontrollore durante il tempo di integrazione ottica, l'acquisizione segue un approccio basato sulla disponibilità del dato. Quando il sensore segnala la presenza di una nuova misura, il firmware imposta la relativa flag di acquisizione; la lettura effettiva viene poi eseguita nel ciclo principale. In questo modo la callback rimane leggera e il main loop può procedere con la lettura della misura tramite \texttt{VL53L1\_GetRangingMeasurementData()} e con il successivo riavvio dell'acquisizione tramite \texttt{VL53L1\_ClearInterruptAndStartMeasurement()}.
...
\subsection{BNO055 — Acquisizione dell'assetto}
Il BNO055 è responsabile del monitoraggio dell'assetto tridimensionale del velivolo. Il firmware utilizza il driver \textit{platform-agnostic} fornito da Bosch Sensortec, adattato al contesto HAL per le funzioni di comunicazione $I^{2}C$, di temporizzazione e di gestione del reset.

La sequenza di inizializzazione del sensore comprende: la verifica del corretto collegamento del dispositivo, la configurazione dei parametri interni, l'impostazione del remap degli assi e dei relativi segni, e l'impostazione della modalità operativa. Come anticipato nel Capitolo~\ref{ch:hardware}, è stata scelta la modalità di fusion NDOF, in cui l'algoritmo proprietario del sensore combina i dati provenienti da accelerometro, giroscopio e magnetometro per restituire una stima stabile e robusta dell'orientamento assoluto.
...
Le letture periodiche degli angoli avvengono nel ramo IMU–servo a 50 [Hz], frequenza coerente con la massima banda passante della \textit{sensor fusion} interna al BNO055 in modalità NDOF. I valori di roll, pitch e yaw vengono mantenuti internamente come grandezze angolari e, al momento della composizione della telemetria, vengono convertiti in centesimi di grado. Questa rappresentazione consente di trasmettere informazione angolare con precisione di 0.01° utilizzando soli interi nella stringa di telemetria.
...
Il diagramma di Figura~\ref{fig:flow_vl53l1x} riassume la sequenza di inizializzazione e di acquisizione del sensore di quota. Il flusso si conclude con l'attivazione della modalità di misura continua, dopodiché il main loop interroga periodicamente la disponibilità del dato senza bloccarsi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/03_firmware/flow_vl53l1x.png}
...
\begin{figure}[H]
    ...
    \caption{Diagramma di flusso del sensore BNO055}
    \label{fig:flow_bno055}
\end{figure}

\subsection{Diagramma di flusso del firmware complessivo}
Il diagramma di Figura~\ref{fig:flow_firmware} sintetizza la cooperazione tra i moduli software. Il flusso si articola in due macro-fasi:
\begin{enumerate}
    \item \textbf{Inizializzazione e safe startup}: il sistema attende un trigger esterno di sicurezza (User button) prima di avviare l'alimentazione ai sensori e di generare i segnali di armamento per gli ESC dei motori.
    \item \textbf{Main loop di controllo}: il ciclo infinito gestisce due percorsi asincroni e indipendenti. Il primo, attivato dalla scadenza di TIM7, gestisce la correzione di assetto tramite i servomotori (roll e pitch) e la correzione differenziale di yaw sui motori. Il secondo, attivato dalla disponibilità di un nuovo dato dal ToF, gestisce la portanza verticale dei motori principali. La separazione di questi due percorsi assicura che un eventuale ritardo di un sensore non comprometta la reattività dell'intero sistema.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/03_firmware/flow_complessivo.png}
    \caption{Diagramma di flusso complessivo del firmware}
    \label{fig:flow_firmware}
\end{figure}
...
La UART è configurata in modalità \textit{full-duplex} asincrona, con 8 bit di dato, 1 bit di stop e nessuna parità, così da sostenere sia l'invio dei comandi da MATLAB sia la trasmissione periodica della telemetria. La ricezione dei comandi è gestita direttamente nel firmware tramite la callback UART: i byte ricevuti vengono accumulati in un buffer fino al carattere di terminazione, \texttt{\textbackslash n} oppure \texttt{\textbackslash r}; una volta completata la riga, il comando viene interpretato e validato.
...
\begin{itemize}
    \item \textbf{CMD,top,bottom,roll,pitch} — comando di attuazione utilizzato durante le prove sperimentali. I quattro campi numerici rappresentano, nell'ordine: PWM del motore superiore, PWM del motore inferiore, PWM del servo di roll, PWM del servo di pitch. Tutti i valori sono espressi in unità CCR (\textit{Capture-Compare Register}) e vengono saturati in lettura entro i limiti hardware definiti in \texttt{config.h}. In particolare, \texttt{LOWER\_LIMIT\_MOTOR} e \texttt{UPPER\_LIMIT\_MOTOR} delimitano l'intervallo PWM ammesso per gli ESC dei motori, mentre \texttt{LOWER\_LIMIT\_SERVO}, \texttt{CENTER\_SERVO} e \texttt{UPPER\_LIMIT\_SERVO} definiscono rispettivamente il limite inferiore, la posizione neutra e il limite superiore dei servomotori associati ai flap. Le costanti \texttt{CCR\_PER\_DEGREE} e \texttt{MAX\_FLAP\_ANGLE\_DEG} permettono invece di collegare il comando PWM alla corrispondente escursione angolare dei flap, rendendo esplicito il vincolo meccanico imposto agli attuatori. I valori dei servomotori vengono aggiornati dopo la saturazione, mentre i valori dei motori vengono applicati solo se i motori risultano abilitati e non è presente una condizione di \textit{kill} latched.

\begin{lstlisting}[caption={Parsing del comando CMD e saturazione dei valori PWM ricevuti da MATLAB.}, label={lst:nome}]
else if (strncmp(rx_buffer, "CMD,", 4) == 0) {

    int top, bottom, roll, pitch;

    if (sscanf(rx_buffer, "CMD,%d,%d,%d,%d",
               &top, &bottom, &roll, &pitch) == 4) {
...
\end{lstlisting}
    ...
    \item \textbf{MOTOR\_ON} — richiede al firmware di passare dallo stato disarmato allo stato armato. In risposta, il firmware abilita la possibilità di applicare i successivi comandi PWM ai motori, mantenendo inizialmente i valori al limite inferiore, e risponde con \texttt{ACK\_ON}.

    \item \textbf{MOTOR\_OFF} — richiede il ritorno allo stato disarmato. Il firmware forza i motori al PWM minimo, risponde con \texttt{ACK\_OFF} e disabilita l'esecuzione dei comandi CMD successivi fino al prossimo MOTOR\_ON.

    \item \textbf{KILL} — comando di emergenza che ferma immediatamente i motori, indipendentemente dallo stato corrente, e attiva una condizione di arresto \textit{latched}. In tale stato i comandi motore non possono essere applicati fino a una nuova abilitazione esplicita del sistema. Il firmware risponde con \texttt{ACK\_KILL}.
\end{itemize}
...
\subsection{ACK, watchdog e diagnostica UART}
Il sistema di \textit{acknowledgement} consente a MATLAB di verificare che i comandi inviati siano stati ricevuti e interpretati correttamente dal firmware. A ogni comando di stato il firmware associa una risposta specifica: \texttt{ACK\_ON} per l'abilitazione dei motori, \texttt{ACK\_OFF} per lo spegnimento controllato, \texttt{ACK\_KILL} per l'arresto di emergenza e \texttt{ACK\_ERR} per righe non valide o condizioni di errore.

\begin{lstlisting}[caption={Enumerazioni utilizzate per gli acknowledge e per la codifica delle cause di arresto.}, label={lst:nome}]
typedef enum {
    ACK_NONE = 0,
    ACK_ON,
    ACK_OFF,
...
Gli \textit{acknowledge} vengono gestiti tramite una variabile di stato dedicata e trasmessi nel ciclo principale solo quando la UART risulta libera. In questo modo la risposta al comando non viene inviata direttamente all'interno della callback di ricezione, mantenendo leggera la ISR e riducendo il rischio di interferenze con la trasmissione periodica della telemetria. Quando è presente un \textit{acknowledge} pendente, esso viene inviato con priorità rispetto alla telemetria, così da permettere a MATLAB di sincronizzarsi rapidamente con lo stato interno del firmware.

A protezione della comunicazione è inoltre previsto un \textit{watchdog} sui comandi provenienti da MATLAB. Quando i motori sono abilitati, il firmware verifica che i comandi di attuazione continuino ad arrivare entro una finestra temporale prefissata. Se tale condizione non viene rispettata, la mancanza di aggiornamenti viene interpretata come possibile perdita di comunicazione o blocco lato MATLAB; di conseguenza, il firmware forza i motori al valore minimo e aggiorna la causa dell'arresto.

La diagnostica UART permette inoltre di distinguere diverse condizioni anomale legate alla comunicazione seriale. Il contatore \texttt{uart\_bad\_lines} tiene traccia delle righe ricevute che non rispettano il formato previsto dal protocollo, \texttt{uart\_rx\_overflows} segnala eventuali saturazioni del buffer di ricezione, mentre \texttt{uart\_rx\_errors} registra errori hardware della periferica UART. Queste informazioni vengono incluse nella telemetria e consentono di riconoscere, durante l'analisi dei dati, se un comportamento anomalo del sistema sia dovuto alla dinamica fisica del DPDF oppure a un problema di comunicazione.

Nel complesso, il protocollo seriale non si limita a trasferire comandi numerici da MATLAB al firmware, ma introduce un livello di controllo sullo stato della comunicazione. Gli \textit{acknowledge}, il \textit{watchdog} e i contatori diagnostici permettono di rendere osservabile e verificabile l'interazione tra PC e microcontrollore durante l'intera prova sperimentale.

%--------------------------------------------------------------
\section{Telemetria robusta a frequenza fissa}
La telemetria rappresenta uno degli elementi centrali del firmware sviluppato, poiché consente di trasformare il DPDF in una piattaforma sperimentale osservabile. L'obiettivo non è soltanto trasmettere le grandezze misurate, ma generare un flusso dati periodico, coerente e diagnosticabile, utilizzabile nelle successive fasi di identificazione, modellazione e simulazione in MATLAB.

Nel firmware, il periodo nominale della telemetria è definito dalla macro \texttt{TELEMETRY\_PERIOD\_MS}, posta pari a 20 [ms]. Tale valore corrisponde a una frequenza di 50 [Hz], scelta per acquisire campioni sufficientemente ravvicinati nel tempo senza saturare eccessivamente il canale seriale. La dimensione massima della stringa trasmessa è invece definita da \texttt{STANDARD\_MESSAGE\_LENGTH}, mentre \texttt{TELEMETRY\_MAX\_PENDING} limita il numero massimo di campioni che possono rimanere in attesa di trasmissione.

La generazione del \textit{tick} telemetrico è affidata al timer TIM3. A ogni interrupt periodico, il firmware non costruisce né trasmette direttamente la stringa dati, ma aggiorna il tempo della telemetria e incrementa il numero di campioni pendenti. Questa scelta mantiene leggera la routine di interrupt, evitando di eseguire all'interno della ISR operazioni più onerose come la formattazione della stringa CSV o la trasmissione UART.

La trasmissione effettiva viene gestita nel ciclo principale. Il firmware procede all'invio solo se sono presenti campioni pendenti, se la UART non è occupata e se non ci sono messaggi di \textit{acknowledge} da trasmettere con priorità. Se la UART rimane occupata o la trasmissione DMA precedente non è ancora conclusa, i campioni vengono accumulati fino al limite definito da \texttt{TELEMETRY\_MAX\_PENDING}. Al superamento di tale limite viene incrementato il contatore \texttt{telemetry\_overrun}, successivamente inserito nella telemetria stessa. In questo modo eventuali problemi di temporizzazione o saturazione del canale seriale diventano informazioni diagnostiche disponibili durante l'analisi dei dati.

La riga telemetrica viene costruita in formato CSV tramite \texttt{snprintf()} e contiene sia le grandezze fisiche del sistema sia informazioni relative ai comandi applicati e allo stato diagnostico del firmware. Il formato utilizzato è il seguente:

\begin{lstlisting}[caption={Composizione della riga telemetrica e trasmissione non bloccante tramite DMA. }, label={lst:nome}]
if (telemetry_pending > 0 &&
    !uart_tx_busy &&
...
La presenza di campi diagnostici come \texttt{uart\_rx\_errors}, \texttt{uart\_rx\_overflows}, \texttt{uart\_bad\_lines}, \texttt{telemetry\_overrun} e \texttt{stop\_reason} permette di distinguere un comportamento fisico del drone da un eventuale problema di comunicazione o acquisizione. In questo modo ogni prova sperimentale può essere analizzata mettendo in relazione lo stato del DPDF, i comandi effettivamente applicati e la qualità del collegamento seriale.

La trasmissione della riga avviene tramite DMA, utilizzando \texttt{HAL\_UART\_Transmit\_DMA()}. Questa scelta evita di bloccare il ciclo principale durante l'invio dei dati seriali: una volta avviata la trasmissione, il firmware imposta la variabile \texttt{uart\_tx\_busy} e continua la propria esecuzione. Al termine dell'invio, la callback di completamento riporta \texttt{uart\_tx\_busy} a \texttt{false}, rendendo nuovamente disponibile il canale UART.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/03_firmware/tim3_config.png}
    \caption{Configurazione di TIM3 per la generazione del tick telemetrico}
    \label{fig:tim3_config}
\end{figure}

Nel complesso, la telemetria implementata non si limita a inviare periodicamente misure sensoriali, ma costituisce un meccanismo di osservazione e diagnostica del test. La temporizzazione tramite TIM3, l'accumulo controllato dei campioni pendenti, lo \textit{snapshot} coerente delle variabili, la trasmissione DMA e l'inclusione di informazioni diagnostiche permettono di ottenere dataset più affidabili e interpretabili per le successive elaborazioni in MATLAB.

%--------------------------------------------------------------
\section{Safe startup e logiche di arresto}
L'avvio del firmware è gestito tramite una sequenza di \textit{safe startup}, eseguita prima della fase operativa vera e propria. Lo scopo di questa procedura è portare il sistema in una condizione iniziale prevedibile, evitando che i motori ricevano comandi non desiderati durante il transitorio di accensione e inizializzazione.
...
Operativamente, la sequenza di \textit{safe startup} può essere suddivisa nelle seguenti fasi:
\begin{enumerate}
    \item inizializzazione delle periferiche e delle variabili interne del firmware;
    \item forzatura dei comandi motore al valore PWM minimo;
    \item posizionamento dei servomotori dei flap nella configurazione neutra;
    \item mantenimento temporizzato del segnale PWM minimo, necessario per consentire l'armamento corretto degli ESC;
    \item segnalazione diagnostica tramite LED durante l'attesa di stabilizzazione;
    \item passaggio alla modalità operativa solo al termine della procedura e in assenza di condizioni di errore o arresto \textit{latched}.
\end{enumerate}


Terminata la fase di avvio, il firmware mantiene comunque attivi diversi livelli di protezione. I valori PWM ricevuti da MATLAB vengono applicati ai motori solo se questi risultano abilitati e se non è presente una condizione di arresto \textit{latched}. In caso contrario, il firmware forza entrambi i motori al valore minimo, impedendo che un comando precedente o non valido possa mantenere attiva la propulsione.

La procedura di spegnimento immediato dei motori è centralizzata nella funzione
\begin{center}
     \texttt{motor\_force\_off\_immediate()}, 
\end{center}
...
Il firmware prevede diversi meccanismi di arresto. Il comando \texttt{MOTOR\_OFF} esegue uno spegnimento controllato, mentre il comando \texttt{KILL} rappresenta un arresto di emergenza e imposta una condizione \textit{latched} che impedisce la riattivazione dei motori senza una nuova abilitazione esplicita. A questi comandi software si aggiunge il pulsante fisico della scheda, che consente di interrompere la prova anche indipendentemente dal canale MATLAB-UART.

Le principali cause di arresto o anomalia vengono codificate nella variabile \texttt{stop\_reason}, trasmessa successivamente in telemetria. Questo permette di distinguere, durante l'analisi dei dati, una prova terminata volontariamente da una prova interrotta per errore UART, overflow del buffer, timeout dei comandi o intervento di emergenza.

Nel complesso, la \textit{safe startup} e le logiche di arresto completano il protocollo di comando e la telemetria, garantendo che il firmware possa riportare il DPDF in una condizione sicura sia all'avvio sia durante l'esecuzione dei test sperimentali.
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
duration     = 30;       
	...
	\begin{itemize}
		...
		\item \texttt{STEP\_SMALL = 50}, \texttt{STEP\_MEDIUM = 100} e \texttt{SIN\_AMP = 100} sono le ampiezze nominali delle sollecitazioni (espresse in microsecondi di PWM per i motori e in punti di deflessione per i flap). Tali costanti vengono utilizzate all'interno della funzione locale MATLAB \texttt{generateSignal} per calcolare istante per istante il profilo del segnale scelto. I valori numerici così generati vengono poi impacchettati dalla Ground Station e trasmessi all'STM32 tramite il comando seriale \texttt{CMD, top, bottom, roll, pitch}. Questi parametri determinano il rapporto segnale--rumore dell'identificazione: ampiezze troppo piccole non producono uno spostamento angolare distinguibile dal rumore del BNO-055, mentre ampiezze eccessive potrebbero forzare le funi di vincolo oltre il loro regime elastico o portare i servomotori in saturazione meccanica.
		...
		\item \texttt{DT\_TARGET = 0.02} sancisce il passo di campionamento temporale dell'intero sistema. Coordinare la telemetria alla stessa frequenza a cui opera il ramo IMU/servo del firmware (50 [Hz]) è una scelta metodologica essenziale per evitare effetti di \textit{aliasing} e sfasamenti nei modelli identificati.
	\end{itemize}
	...
	Per risolvere questa situazione, lo script MATLAB implementa un meccanismo di \texttt{keep-alive} attivo esclusivamente durante le fasi di transizione, ovvero quando la Ground Station è in attesa bloccante degli ACK critici (come l'avvio o l'arresto della prova). In questi frangenti, per evitare che l'attesa interrompa il flusso dati verso il velivolo facendo scattare il watchdog di bordo, vengono inviati periodicamente comandi neutri a bassa frequenza. Viceversa, quando il drone si trova in stato disarmato fuori dalle finestre di test e non vi è alcuno scambio di messaggi in corso, lo script rimane in uno stato di \textit{idle} completo senza inviare alcuna stringa sulla seriale, azzerando il rischio di attuazioni spurie. L'implementazione è inserita nella funzione \texttt{sendCommandWaitAck}:
...
Durante le sessioni sperimentali abbiamo riscontrato che il velivolo, vincolato alle corde di sicurezza, tendeva a innescare oscillazioni divergenti improvvise e pericolose. Si è deciso quindi di implementare una gestione coordinata degli arresti di sicurezza, strutturata su due livelli a priorità differenziata per garantire l’integrità strutturale del prototipo e la sicurezza dell'operatore:
\begin{enumerate}
    \item \textbf{Arresto controllato (Safe Stop):} viene innescato al naturale scadere della durata della prova (ad esempio dopo 30~[s]) oppure automaticamente dal firmware (Watchdog) in caso di perdita del segnale di \textit{keep-alive}. Lo script invia il comando \texttt{MOTOR\_OFF}, portando gradualmente i motori al PWM minimo e i servomotori in posizione neutra per un'uscita controllata dallo stato armato.
    \item \textbf{Terminazione d'emergenza (Kill):} viene attivata manualmente da remoto dall'operatore tramite la pressione asincrona della barra spaziatrice o dell'apposito pulsante sulla GUI di MATLAB\@. Questo comando interrompe istantaneamente qualsiasi routine in corso e azzera immediatamente i segnali di attuazione.
\end{enumerate}
...
function keyKill(~, event)
	...
	\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/04_telemetria/kill_switch.png}
    \caption{Interfaccia grafica del Kill Switch}
    \label{fig:tim3_config}
\end{figure}
	...
	\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/04_telemetria/mod_controllo.png}
    \caption{Menù di selezione del profilo di eccitazione}
    \label{fig:tim3_config}
\end{figure}
...
Vale la pena anticipare che il modello LTI così ottenuto è valido localmente,
nell'intorno operativo effettivamente esplorato durante le prove di identificazione.
In particolare, i dataset PRBS sono stati acquisiti mantenendo i motori attorno alla
condizione nominale di prova e imponendo ai \textit{flap} variazioni limitate rispetto
alla posizione neutra. Poiché la conversione adottata è pari a
\(K_\text{CCR}=10~\mathrm{CCR/deg}\), una variazione di 50--100 CCR corrisponde
a circa 5--10 gradi di deflessione del \textit{flap}. I modelli identificati descrivono
quindi la dinamica del DPDF nel range di piccole e medie perturbazioni utilizzato
per l'identificazione, e non devono essere estesi a condizioni di grande angolo,
saturazione dei \textit{flap} o funzionamento lontano dal punto di equilibrio. Le dinamiche globali del velivolo --- inclusi gli accoppiamenti tra gli assi, gli effetti delle saturazioni meccaniche e l'interazione con il vincolo elastico delle funi --- saranno invece esplorate tramite il simulatore fisico tridimensionale descritto nel Capitolo~\ref{ch:simulazione}, che adotta un modello a parametri concentrati di tipo \textit{grey-box} e complementa le funzioni di trasferimento qui identificate.
...
\begin{figure}[H]
	...
	\caption{\textit{Dataset} sperimentale ad anello aperto ottenuto tramite stimolazione PRBS. In alto: comandi PWM ai motori; al centro: comandi PWM ai \textit{flap}; in basso: risposta angolare del velivolo misurata dall'IMU. La figura superiore si riferisce all'asse di \textit{roll}, quella inferiore all'asse di \textit{pitch}.}
...
In questo modo l'ingresso del modello non è più espresso in unità di registro, ma in gradi di deflessione del \textit{flap} rispetto alla posizione neutra. Pertanto, nei modelli identificati, $\Delta u(t)$ rappresenta la variazione angolare del \textit{flap} comandata al servomotore.
...
I dataset utilizzati per l'identificazione sono costituiti dai campioni telemetrici effettivamente salvati a 50~[Hz]. Di conseguenza, anche qualora alcune grandezze interne al firmware vengano aggiornate a frequenze differenti, il modello identificato rappresenta la dinamica osservata alla cadenza temporale dei dati disponibili, pari a $T_s = 0.02$~[s].

%--------------------------------------------------------------
\section{Selezione dell'architettura del modello}
\label{sec:architettura_modello}

Il \textit{DPDF}, considerato nella sua interezza, è un sistema multi-ingresso multi-uscita (MIMO), poiché dispone di più attuatori --- motori e \textit{flap} --- e produce più grandezze misurabili, quali \textit{roll}, \textit{pitch}, \textit{yaw} e quota. Ai fini della sintesi preliminare dei regolatori di assetto, il problema è stato tuttavia ricondotto all'identificazione di due canali SISO equivalenti, uno per l'asse di \textit{roll} e uno per l'asse di \textit{pitch}.

In ciascuna prova di identificazione è stato eccitato prevalentemente un solo \textit{flap} alla volta, mantenendo gli altri ingressi in condizioni nominali o costanti. Per l'asse di \textit{roll} l'ingresso del modello è quindi la variazione angolare del \textit{flap} di \textit{roll} rispetto alla posizione neutra, mentre l'uscita è la variazione dell'angolo di \textit{roll} misurato dalla IMU. In modo analogo, per l'asse di \textit{pitch} l'ingresso è la variazione angolare del \textit{flap} di \textit{pitch} e l'uscita è la variazione dell'angolo di \textit{pitch}.

I modelli ottenuti devono quindi essere interpretati come approssimazioni locali dei canali principali di assetto, e non come una descrizione completa della dinamica MIMO del \textit{DPDF}.

La stima è stata condotta nel dominio del tempo continuo tramite il \textit{System Identification Toolbox} di MATLAB, in particolare mediante la funzione \texttt{tfest}. Sebbene i dati sperimentali siano acquisiti a tempo discreto con periodo di campionamento $T_s = 0.02$~[s], le funzioni di trasferimento riportate in questa sezione sono espresse nel dominio continuo $s$. La discretizzazione rilevante ai fini dell'implementazione firmware viene discussa successivamente nella sezione dedicata al controllore.
...
\begin{itemize}
	\item \textbf{Asse di \textit{roll}}: modello a 2 poli e 1 zero ($n=2$, $m=1$), che cattura adeguatamente la dinamica inerziale del velivolo nella sezione trasversale, mantenendo una struttura sufficientemente semplice.
	\item \textbf{Asse di \textit{pitch}}: modello a 3 poli e 1 zero ($n=3$, $m=1$). Il polo aggiuntivo rispetto all'asse di \textit{roll} è stato introdotto perché i modelli di ordine inferiore non riproducevano con sufficiente accuratezza il transitorio osservato lungo l'asse longitudinale. Tale maggiore complessità può essere attribuita alla distribuzione non ideale delle masse, all'interazione con le funi di vincolo e alla non perfetta uniformità del flusso aerodinamico all'interno del condotto.
\end{itemize}
...
\begin{table}[H]
	...
	\begin{tabular}{l c c}
		...
		\bottomrule
	\end{tabular}
	\caption{Coefficienti numerici delle funzioni di trasferimento identificate.}
...
\begin{equation}
	\text{Fit} = 100 \cdot \left( 1 -
	\frac{\sqrt{\sum_{k=1}^{N} \bigl| y(k) - \hat y(k) \bigr|^{2}}}
	{\sqrt{\sum_{k=1}^{N} \bigl| y(k) - \bar y \bigr|^{2}}}
	\right) \quad [\%]
	\label{eq:nrmse}
\end{equation}
...
I valori di \textit{fit} ottenuti sui \textit{dataset} PRBS si riferiscono ai modelli selezionati al termine del confronto tra le strutture candidate, ossia il modello a 2 poli e 1 zero per l'asse di \textit{roll} e il modello a 3 poli e 1 zero per l'asse di \textit{pitch}:
\begin{itemize}
	\item asse di \textit{roll}, modello a 2 poli e 1 zero: $\text{Fit} = 77.85\%$;
	\item asse di \textit{pitch}, modello a 3 poli e 1 zero: $\text{Fit} = 77.67\%$.
\end{itemize}

Si tratta di valori soddisfacenti per un'identificazione \textit{black-box} su un sistema dotato di non linearità importanti e testato in condizioni vincolate. La quota residua di varianza non spiegata dal modello è imputabile a effetti che un modello lineare SISO equivalente non può catturare integralmente: in particolare l'attrito non lineare sui perni dei \textit{flap}, l'azione non perfettamente simmetrica delle funi di vincolo e i piccoli accoppiamenti tra gli assi. I modelli identificati devono quindi essere interpretati come approssimazioni locali della dinamica dominante del \textit{DPDF} nella configurazione sperimentale adottata, sufficienti per la sintesi preliminare dei regolatori PIDF e per la successiva verifica in simulazione.
...
\begin{figure}[H]
	...
	\caption{Analisi di \textit{cross-validazione} nel dominio del tempo per i due assi. Confronto tra risposta angolare reale del prototipo (linea nera) e uscita stimata dal modello LTI (linea colorata). In alto l'asse di \textit{roll} (Fit $77.85\%$), in basso l'asse di \textit{pitch} (Fit $77.67\%$).}
...
I test condotti hanno restituito, per entrambi gli assi, un guadagno statico di \textbf{segno negativo} ($\mu < 0$). Questo risultato fornisce una conferma fisica del modello stimato: un incremento del PWM del servomotore --- corrispondente a un'inclinazione positiva del \textit{flap} secondo la convenzione del firmware --- genera una coppia aerodinamica che ruota il drone nel verso opposto rispetto al sistema di riferimento destrorso solidale all'IMU.

Il segno negativo del guadagno statico non rappresenta quindi un'anomalia del processo di identificazione, ma riflette la convenzione geometrica adottata tra verso positivo del comando \textit{flap} e verso positivo dell'angolo misurato dalla IMU. Per questo motivo, nella successiva implementazione, i guadagni PID possono essere mantenuti positivi, demandando al \textit{mixer} firmware l'inversione del verso di attuazione.

%--------------------------------------------------------------
\section{Il regolatore PID: principi di funzionamento}
\label{sec:principi_pid}

Prima di descrivere la procedura di sintesi automatica, è opportuno richiamare il principio di funzionamento del regolatore impiegato. Il \textit{DPDF} è un sistema intrinsecamente instabile attorno alla condizione di assetto orizzontale: in assenza di azione correttiva, il momento ribaltante indotto dall'eccentricità del baricentro (Sezione~\ref{sec:eccentricita_cg}) fa divergere gli angoli di \textit{roll} e \textit{pitch}. Il compito del controllore è chiudere un anello di retroazione che, confrontando istante per istante l'assetto misurato dall'IMU con il riferimento desiderato, generi un comando ai \textit{flap} tale da riportare il velivolo verso l'equilibrio.

Si definisce a tal fine il segnale di \textbf{errore} come differenza tra l'angolo di riferimento $\theta_\text{rif}$ e l'angolo misurato $\theta(t)$:
\begin{equation}
	e(t) = \theta_\text{rif} - \theta(t)
	\label{eq:errore_pid}
\end{equation}
Nelle prove di stabilizzazione il riferimento è nullo ($\theta_\text{rif} = 0\degree$), poiché l'obiettivo è mantenere il drone in assetto orizzontale; l'errore coincide dunque, a meno del segno, con l'inclinazione istantanea del velivolo. A partire da questo errore, il regolatore PID elabora il comando di controllo combinando tre azioni distinte, ciascuna con un significato fisico preciso.

\subsection{Le tre azioni di controllo}
\label{sec:tre_azioni}

Nella sua forma ideale a tempo continuo, l'uscita di un regolatore PID è la somma di un termine proporzionale, uno integrale e uno derivativo dell'errore:
\begin{equation}
	u(t) = \underbrace{K_p \, e(t)}_{\text{proporzionale}}
	\; + \; \underbrace{K_i \int_{0}^{t} e(\tau)\, d\tau}_{\text{integrale}}
	\; + \; \underbrace{K_d \, \frac{d e(t)}{d t}}_{\text{derivativo}}
	\label{eq:pid_ideale}
\end{equation}
I tre guadagni $K_p$, $K_i$, $K_d$ pesano il contributo di ciascuna azione e costituiscono i parametri da determinare in fase di taratura.

\paragraph{Azione proporzionale.}
Il termine $K_p \, e(t)$ reagisce istantaneamente all'errore, generando un comando tanto maggiore quanto più elevato è lo scostamento tra riferimento e misura. Questa azione costituisce il contributo principale di richiamo del sistema, ma un controllo puramente proporzionale non garantisce in generale l'annullamento dell'errore a regime. Infatti, man mano che l'errore diminuisce, si riduce anche l'azione di controllo generata dal termine proporzionale; può quindi permanere uno scostamento tra il valore desiderato e il valore effettivamente raggiunto, necessario a produrre il comando richiesto all'attuatore.

Questo effetto è ancora più evidente in presenza di disturbi costanti, come il momento ribaltante dovuto all'eccentricità del baricentro o eventuali squilibri aerodinamici. L'eliminazione dell'errore statico è affidata al termine integrale del PID.

\paragraph{Azione integrale.}
Il termine $K_i \int e\, d\tau$ accumula nel tempo l'errore e ne integra la storia passata. La sua funzione è proprio quella di \textbf{annullare l'errore a regime}: finché permane un errore, anche piccolo, l'integrale continua a crescere aumentando la correzione, fino a quando il contributo di controllo è sufficiente a compensare lo scostamento statico residuo. Nel caso del \textit{DPDF} questa azione è essenziale per compensare lo sbilanciamento statico del baricentro.

Il prezzo da pagare è il rischio di \textit{windup}: se l'attuatore satura (i \textit{flap} raggiungono il limite di $\pm 30\degree$) e l'errore persiste, l'integrale può accumulare valori molto elevati, generando sovraelongazioni e ritardi di recupero. Per questo motivo l'azione integrale è dotata di un meccanismo \textbf{anti-windup} a saturazione, descritto nel seguito.

\paragraph{Azione derivativa.}
Il termine $K_d \, \dot e(t)$ reagisce alla \textit{velocità} di variazione dell'errore, anticipando la dinamica del sistema. Esso introduce uno \textbf{smorzamento} che frena le oscillazioni e riduce la sovraelongazione, opponendosi alle variazioni rapide di assetto. La derivazione pura è però estremamente sensibile al rumore di misura: la derivata del rumore del BNO-055 produrrebbe comandi ad alta frequenza e ampiezza ai servomotori, con conseguente \textit{chattering}. Per questa ragione, nell'implementazione effettiva (Sezione~\ref{sec:sintesi_pidf}) l'azione derivativa è asservita a un filtro passa-basso del primo ordine, dando luogo alla forma PIDF di equazione~\eqref{eq:pidf}.

\subsection{Implementazione discreta in forma posizionale}
\label{sec:pid_posizionale}

Il regolatore non opera nel continuo ma viene eseguito come algoritmo a tempo discreto alla frequenza nominale di 50~[Hz] ($T_s = 0.02$~[s]). Coerentemente con l'architettura event-driven descritta nel Capitolo~\ref{ch:firmware}, la cadenza temporale del regolatore è determinata dal timer periodico, mentre il calcolo dell'azione di controllo può essere eseguito nel ciclo principale a seguito dell'attivazione della relativa flag. In questo modo la routine di interrupt rimane leggera e l'elaborazione numerica viene mantenuta nel contesto sequenziale del \textit{main loop}.

L'implementazione adottata è in \textbf{forma posizionale} (\textit{positional form}): a ogni passo di campionamento $k$ l'azione di controllo viene \textit{ricalcolata per intero} a partire dall'errore corrente, dall'accumulatore integrale e dalla derivata filtrata. Questa scelta si distingue dalla forma \textit{incrementale} (\textit{velocity form}), nella quale si calcola invece l'incremento $\Delta u[k]$ rispetto al comando precedente; la forma posizionale è stata preferita per la maggiore trasparenza nella gestione esplicita della saturazione dell'attuatore e del termine anti-windup.

Le tre azioni sono discretizzate come segue. Il termine proporzionale è immediato:
\begin{equation}
	P[k] = K_p \, e[k]
	\label{eq:pid_p}
\end{equation}

L'azione integrale è realizzata mediante un accumulatore aggiornato a ogni passo e limitato (\textit{clamping}) entro l'escursione fisica del flap, così da impedire il \textit{windup}:
\begin{equation}
	I[k] = \mathrm{sat}\!\Bigl( I[k-1] + K_i \, T_s \, e[k] \Bigr), 
	\qquad |I[k]| \leq 30\degree
	\label{eq:pid_i}
\end{equation}

L'azione derivativa filtrata è discretizzata con la trasformazione di Eulero all'indietro introdotta in equazione~\eqref{eq:euler_backward}, da cui discende il coefficiente $\alpha = T_s/(T_s + T_f)$:
\begin{equation}
	D[k] = (1-\alpha)\, D[k-1] + \alpha \, \frac{K_d}{T_s} \bigl( e[k] - e[k-1] \bigr)
	\label{eq:pid_d}
\end{equation}

Con questa convenzione, valori elevati di $\alpha$ aumentano il peso del termine derivativo istantaneo e rendono il filtro più reattivo; valori più bassi aumentano invece il peso della memoria del valore precedente, producendo un filtraggio più incisivo.

Il comando complessivo applicato al servomotore è infine la somma delle tre azioni, a sua volta saturata al limite meccanico dei \textit{flap}:
\begin{equation}
	u[k] = \mathrm{sat}\!\Bigl( P[k] + I[k] + D[k] \Bigr),
	\qquad |u[k]| \leq 30\degree
	\label{eq:pid_posizionale}
\end{equation}

Il comando $u[k]$, espresso in gradi di deflessione del \textit{flap}, viene poi convertito nel corrispondente valore CCR tramite la costante di conversione $K_\text{CCR} = 10.0$~[CCR/deg] (Sezione~\ref{sec:pre_elaborazione}) prima di essere inviato al timer di generazione del segnale PWM:
\begin{equation}
	CCR = CCR_\text{neutro} + K_\text{CCR} \, u[k]
	\label{eq:conversione_ccr_pid}
\end{equation}

Poiché i guadagni sono stati identificati lavorando in unità fisiche di gradi (Sezione~\ref{sec:architettura_modello}), i valori $K_p$, $K_i$, $K_d$ sintetizzati in MATLAB sono direttamente trasferibili nei campi del file \texttt{config.c} senza ulteriori riscalature, salvo l'eventuale gestione del segno demandata al \textit{mixer}.
...
È importante osservare che i regolatori PIDF descritti in questa sezione non sono stati utilizzati durante le prove di identificazione \textit{open-loop}. In quella fase i segnali PWM sono stati generati da MATLAB e applicati direttamente agli attuatori, entro i limiti di sicurezza imposti dal firmware. I regolatori vengono invece sintetizzati a partire dai modelli identificati e costituiscono il risultato della fase di identificazione, destinato alla verifica in simulazione e alla successiva integrazione nel firmware di controllo in anello chiuso.
...
\begin{enumerate}
	\item Il parametro \texttt{DesignFocus} è stato configurato su \texttt{'reference-tracking'} per privilegiare la dolcezza di inseguimento del riferimento ed evitare sovraelongazioni pronunciate; tale impostazione produce guadagni più contenuti rispetto all'alternativa \texttt{'disturbance-rejection'}, più aggressiva ma anche più rischiosa per un primo collaudo.
	\item La verifica preliminare del regolatore è stata effettuata osservando la risposta al gradino del sistema in anello chiuso. Tale analisi consente di valutare rapidità del transitorio, sovraelongazione e assestamento a regime del modello controllato.
\end{enumerate}
...
% Verifica della risposta in anello chiuso
sys_cl = feedback(C_pid * sys_asse, 1);
\end{lstlisting}

La verifica preliminare dei regolatori sintetizzati è stata condotta analizzando la risposta al gradino del sistema in anello chiuso. In questa prova il modello identificato viene chiuso in retroazione con il controllore PIDF ottenuto tramite \texttt{pidtune}, e viene valutata la capacità del sistema controllato di inseguire un riferimento unitario.

Le risposte mostrate in Figura~\ref{fig:step_tracking_pidf} sono normalizzate rispetto al riferimento unitario utilizzato nella simulazione MATLAB. Esse non rappresentano direttamente un angolo fisico imposto al prototipo, ma forniscono una verifica della dinamica relativa del sistema controllato, evidenziando rapidità del transitorio, sovraelongazione e assestamento a regime.

\begin{figure}[H]
	\centering
	
	\includegraphics[width=0.90\textwidth]{figures/05_modello/step_tracking_roll.png}
	
	\vspace{0.4cm}
	
	\includegraphics[width=0.90\textwidth]{figures/05_modello/step_tracking_pitch.png}
	
	\caption{Risposta al gradino in anello chiuso dei modelli identificati controllati tramite PIDF. In alto: asse di \textit{roll}; in basso: asse di \textit{pitch}. Le curve mostrano l'inseguimento di un riferimento unitario da parte del sistema controllato.}
	\label{fig:step_tracking_pidf}
\end{figure}

Per l'asse di \textit{roll} si osserva una risposta più rapida, caratterizzata da una sovraelongazione iniziale più marcata e da un successivo rientro verso il riferimento. Per l'asse di \textit{pitch}, invece, la sovraelongazione risulta più contenuta, ma l'assestamento è più lento. Tale differenza è coerente con i modelli identificati: l'asse di \textit{pitch}, descritto da una funzione di trasferimento di ordine superiore, presenta una dinamica più lenta e articolata rispetto all'asse di \textit{roll}.
...
Il coefficiente $\alpha$ è compreso nell'intervallo $[0,1]$ e, nella forma discreta adottata in equazione~\eqref{eq:pid_d}, determina il peso relativo tra valore derivativo precedente e nuova stima della derivata. Un valore elevato di $\alpha$ assegna maggiore peso al termine derivativo istantaneo e quindi produce un filtraggio più leggero; un valore più basso assegna maggiore peso alla memoria del filtro e produce invece un'azione di smorzamento più marcata. Per il caso in esame i due assi richiedono filtraggi differenti, come risulterà dalla Tabella~\ref{tab:guadagni_pid} riportata nella sezione successiva.
...
I valori riportati nella Tabella~\ref{tab:guadagni_pid} si interpretano nel modo seguente. I guadagni proporzionali e integrali dell'asse di \textit{roll} sono significativamente più elevati di quelli dell'asse di \textit{pitch}, conseguenza diretta dei rispettivi modelli identificati: l'asse di \textit{roll} presenta un guadagno statico più piccolo in modulo, dunque richiede un'azione di controllo proporzionalmente più \textit{forte} per riportare il velivolo nella condizione di riferimento.

Per quanto riguarda i coefficienti del filtro derivativo, il valore $\alpha = 0.27$ per il \textit{roll} indica un filtraggio più incisivo, necessario per attenuare il rumore di assetto presente su quest'asse. Al contrario, il valore $\alpha = 0.94$ per il \textit{pitch} assegna maggiore peso alla derivata istantanea e corrisponde a un filtraggio più leggero, coerente con un canale di misura che, nelle prove analizzate, ha mostrato una dinamica meno rumorosa.

%--------------------------------------------------------------
\section{Considerazioni sull'approccio black-box}
\label{sec:limiti_blackbox}

I modelli identificati nel presente capitolo forniscono una descrizione lineare e tempo-invariante della risposta angolare del velivolo, valida \textit{localmente} attorno alla condizione di equilibrio. Questa rappresentazione è adeguata allo scopo per cui è stata costruita --- la sintesi automatica dei regolatori PID tramite \texttt{pidtune} --- ed è coerente con l'ipotesi di funzionamento del PID stesso, che opera sulla linearizzazione del sistema attorno al riferimento.

Esistono però aspetti della dinamica del \textit{DPDF} che, per loro natura, non possono essere catturati da un modello LTI SISO equivalente. Tra questi: l'accoppiamento tra gli assi di \textit{roll} e \textit{pitch}, particolarmente significativo a grandi inclinazioni; gli effetti delle saturazioni meccaniche dei \textit{flap} e degli ESC; l'interazione non lineare tra il velivolo e le quattro funi di vincolo che lo sostengono nella gabbia di sicurezza; e gli effetti geometrici di grande angolo, in cui le proiezioni della spinta sulle componenti orizzontali e verticali cessano di poter essere approssimate dai loro sviluppi al primo ordine.
...
\begin{itemize}
	\item l'\textbf{identificazione \textit{black-box}} (Capitolo~\ref{ch:modello}) produce un modello LTI dedicato alla \textit{sintesi del controllore} con strumenti consolidati come \texttt{pidtune}, e si concentra sul singolo canale di assetto alla volta;
	\item la \textbf{simulazione fisica \textit{grey-box}} (Capitolo~\ref{ch:simulazione}) fornisce un ambiente di \textit{verifica} in cui i regolatori sintetizzati possono essere testati in condizioni più realistiche, includendo gli effetti non catturati dal modello LTI.
\end{itemize}
...
Si segnala infine, per completezza, una piccola discrepanza temporale tra i due ambienti: l'identificazione lavora con periodo di campionamento $T_s = 0.02$ [s] coerente con la frequenza del firmware, mentre il simulatore del Capitolo~\ref{ch:simulazione} adotta un passo di visualizzazione $\Delta t = 0.025$ [s] suddiviso in più \textit{substep} di integrazione fisica. La scelta è motivata da ragioni di compromesso tra fluidità della visualizzazione 3D e fedeltà numerica dell'integratore, e non altera la validità dei regolatori sintetizzati, che il simulatore continua a eseguire alla loro frequenza nominale.
	...
	Lo \textit{script} \texttt{Sim.m} include una doppia interfaccia grafica. La finestra principale presenta la scena 3D del drone all'interno della gabbia, con visualizzazione in tempo reale di assetto, traiettoria, tensione delle funi e quota; tre grafici laterali mostrano l'evoluzione temporale di quota, angoli di \textit{roll}/\textit{pitch} e angolo di \textit{yaw}. Una seconda finestra dedicata espone gli \textit{slider} di controllo: due per il CCR dei motori superiore e inferiore (range $[750, 1500]$), sei per i guadagni dei PID di \textit{roll} e \textit{pitch} ($K_p$, $K_i$, $K_d$ per asse), oltre ai pulsanti di START, STOP e RESET. Questa configurazione permette di esplorare interattivamente lo spazio dei parametri, osservando immediatamente l'effetto di ogni modifica sul comportamento del sistema --- modalità di lavoro che si è dimostrata particolarmente efficace durante la fase di calibrazione dei parametri fisici.
		...
		\includegraphics[width=0.95\textwidth]{figures/06_simulazione/interfaccia_grafica.png}
	%--------------------------------------------------------------
...
function [Fx, Fy, Fz, Mx, My] = rope_forces(S, C)
	...
	Il comportamento del sistema ad anello chiuso è stato osservato conducendo sessioni di simulazione continuative della durata di 60 [s], a partire dalla configurazione di equilibrio statico del drone appeso alle funi, con i motori al \textit{throttle} di equilibrio ($CCR = 1050$) e i regolatori PID attivi sui due assi di assetto.  Per sollecitare il sistema, durante la sessione rappresentativa riportata in Figura~\ref{fig:simulazione_hover}  è stato imposto un gradino sul comando dei motori da CCR = 1050 a CCR = 1200, applicato dopo un breve intervallo iniziale a regime di equilibrio.	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figures/06_simulazione/screenshot_simulatore.png}
		\caption{Risposta temporale del DPDF nel simulatore (sessione di 60 [s], PID attivo) con parametri PID in configurazione di sicurezza. A t = 30 [s] viene applicato un gradino sul CCR da 1050 a 1200. In alto: quota del bordo inferiore del cilindro, che rimane stabile a circa 300 mm indipendentemente dalla variazione di throttle, a causa del vincolo delle funi. Al centro: angoli di roll (linea rossa) e pitch (linea tratteggiata verde), che mostrano un incremento delle oscillazioni in seguito al gradino, evidenziando come l'aumento di spinta amplifichi il momento dovuto all'offset del centro di massa. In basso: angolo di yaw.}
	...
	\begin{itemize}
		\item \textbf{Il drone permane nella configurazione vincolata}, sostenuto dalle quattro funi che restano in tensione per l'intera durata della prova. La quota del bordo inferiore si stabilizza intorno a 300 [mm] dal pavimento della gabbia, valore corrispondente alla configurazione di equilibrio meccanico in cui la componente verticale della tensione delle funi compensa la differenza tra peso del velivolo e spinta dei motori. Il gradino di throttle non altera sensibilmente questo equilibrio verticale, confermando che la spinta complessiva resta insufficiente per produrre il decollo libero anche al CCR più elevato.

		\item \textbf{Il gradino di throttle destabilizza l'assetto su roll e pitch.} In seguito al gradino da CCR = 1050 a CCR = 1200, gli angoli di roll e pitch non convergono verso lo zero, ma divergono progressivamente con andamento irregolare, raggiungendo escursioni dell'ordine di alcuni gradi senza mostrare segni di ristabilizzazione entro la durata della prova. Tale comportamento evidenzia un'autorità di controllo insufficiente dei flap aerodinamici alle nuove condizioni di throttle, e/o un accoppiamento tra il momento ribaltante generato dall'eccentricità del baricentro e l'azione dei regolatori PID, che non riescono a riportare il sistema in condizione di equilibrio.

		\item \textbf{Lo yaw mostra una deriva monotona crescente} successiva al gradino, in luogo della condizione di quiete attesa. Tale comportamento è coerente con la presenza, nel modello, di un termine di coppia di imbardata (\texttt{Myaw}) legato alla differenza di velocità angolare tra le due eliche controrotanti, che non viene compensato in assenza di un anello di controllo dedicato sull'asse di yaw: l'incremento di CCR si traduce quindi in una coppia di reazione netta non nulla che fa derivare l'angolo di yaw nel tempo.
	\end{itemize}
